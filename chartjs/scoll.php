  ...
  <script>
    ...
    const config = {
      ...
      options: {
        scales: {
          x: {
            min:  window.innerWidth <= 600 ? 0 : 0,
            max:  window.innerWidth <= 600 ? 6 : 9,
          },
          y: {
            beginAtZero: true
          }
    ...
    function moveScroll() {
      ...
      canvas.addEventListener('click', (event) => {
        ...
        if (x >= left - 15 && x <= left + 15 && y >= height / 2 + top - 15 && y <= height / 2 + top + 15) {
          myChart.options.scales.x.min = myChart.options.scales.x.min - (window.innerWidth <= 600 ? 7 : 10);
          myChart.options.scales.x.max = myChart.options.scales.x.max - (window.innerWidth <= 600 ? 7 : 10);

          if (myChart.options.scales.x.max <= 0) {
            myChart.options.scales.x.min = 0;
            myChart.options.scales.x.max = window.innerWidth <= 600 ? 6 : 9;
          }
        }
        ///////////////////////////////////////////////////////////////////////////////////////////////////
        if (x >= right - 15 && x <= right + 15 && y >= height / 2 + top - 15 && y <= height / 2 + top + 15) {
          ...
          if (myChart.options.scales.x.max >= data.datasets[0].data.length) {
            myChart.options.scales.x.min = data.datasets[0].data.length-(window.innerWidth <= 600 ? 7 : 10);
